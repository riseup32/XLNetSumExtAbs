...
class Translator(object):
    ...
    def __init__(self,
        ...
        # self.vocab.add_tokens(['<unused2>', '<unused3>', '<unused4>'])
    ...
    def from_batch(self, translation_batch):
        ...
        for b in range(batch_size):
            ...
            pred_sents = self.vocab.convert_tokens_to_string(pred_sents).replace('<unk>', ' <unk>')
            ...
            raw_src = self.vocab.convert_tokens_to_string(raw_src).replace('<unk>', ' <unk>').replace('<sep>', ' <sep>').replace('<cls>', ' <cls>')
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, gold, src = trans
                    pred_str = pred.replace('<s>', '').replace('<eod>', '').replace('<pad>', '').replace('</s>', '').replace(r' +', ' ').replace(' <mask> ', '<q>').replace('<mask>', '').strip()
                    gold_str = gold.strip()
                    if(self.args.recall_eval):
                        _pred_str = ''
                        gap = 1e3
    ...
    def _fast_translate_batch(self,
        ...
        src_features = self.model.xlnet(src, segs, mask_src)
