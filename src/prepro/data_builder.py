...
# from others.tokenization import XlnetTokenizer
...
def load_xml(p):
    ...
    if (len(paras) > 0):
        if (len(byline) > 0):
            paras = [title + ['<eod>'] + byline + ['<eop>']] + paras
        else:
            paras = [title + ['<eod>']] + paras
...
class XlnetData():
    def __init__(self, args):
        ...
        # self.tokenizer.add_tokens(['<unused2>', '<unused3>', '<unused4>'])
    ...
    def preprocess(self, src, tgt, sent_labels, use_xlnet_basic_tokenizer=False, is_test=False):
...
def format_to_xlnet(args):
    ...
    for corpus_type in datasets:
        ...
        for json_f in glob.glob(pjoin(args.raw_path, '*' + corpus_type + '.*.json')):
            ...
            a_lst.append((corpus_type, json_f, args, pjoin(args.save_path, real_name.replace('json', 'xlnet.pt'))))
        print(a_lst)
        pool = Pool(args.n_cpus)
        for d in pool.imap(_format_to_xlnet, a_lst):
            pass
...
def _format_to_xlnet(params):
    ...
    for d in jobs:
        ...
        b_data = xlnet.preprocess(source, tgt, sent_labels, use_xlnet_basic_tokenizer=args.use_xlnet_basic_tokenizer, \
                                 is_test=is_test)
        # b_data = xlnet.preprocess(source, tgt, sent_labels, use_xlnet_basic_tokenizer=args.use_xlnet_basic_tokenizer)
